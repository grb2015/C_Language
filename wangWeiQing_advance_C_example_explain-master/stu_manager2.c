...
#include <stdlib.h>
#include <string.h>
#include<curses.h>      /* linux中使用 */
//#include <conio.h>   /* windows中使用 */
#define N 100
...
{  
	...
	do
	{
		...
		//do 
	//	{
            yes_no = getchar_and_ignore_n();
            printf("yes_no = %d\n",yes_no);
		                                     /* 问题在于，这里如果输入了Y然按回车，则Y会被yes_no承接到，但是回车却被上报--1处的choose承接到了*/
                                             /* 所以上面必须用一个while循环来忽略掉\n */
	//	}while(yes_no!='Y'|| yes_no!='y' || yes_no!='N'||yes_no!='n');
        printf("break from yes_no");
